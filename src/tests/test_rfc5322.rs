use crate::rfc5322::*;
use crate::rfc5321::{DotString, Mailbox as SMTPMailbox};
...
fn simple_from() {
    ...
    assert_eq!(parsed.address, SMTPMailbox(DotString("jdoe".into()).into(), "machine.example".parse().unwrap()))
}
...
fn simple_sender() {
    ...
    if let Address::Mailbox(Mailbox{dname, address}) = parsed {
        ...
        assert_eq!(address, SMTPMailbox(DotString("mjones".into()).into(), "machine.example".parse().unwrap()))
    } else {
        unreachable!();
    }
}
...
fn simple_reply_to() {
    ...
    assert_eq!(parsed.address, SMTPMailbox(DotString("smith".into()).into(), "home.example".parse().unwrap()))
}
    ...
    assert_eq!(parsed, [Address::Group(Group{
        ...
        members: vec![
            Mailbox { dname: Some("Chris Jones".into()),
                      address: SMTPMailbox(DotString("c".into()).into(), "public.example".parse().unwrap())},
            Mailbox { dname: None,
                      address: SMTPMailbox(DotString("joe".into()).into(), "example.org".parse().unwrap())},
            Mailbox { dname: Some("John".into()),
                      address: SMTPMailbox(DotString("jdoe".into()).into(), "one.test".parse().unwrap())},
        ]
    })]);
}
...
fn multi_reply_to() {
    ...
    assert_eq!(parsed, [
        Address::Mailbox(Mailbox { dname: Some("Mary Smith".into()),
                                   address: SMTPMailbox(DotString("mary".into()).into(), "x.test".parse().unwrap())}),
        Address::Mailbox(Mailbox { dname: None,
                                   address: SMTPMailbox(DotString("jdoe".into()).into(), "example.org".parse().unwrap())}),
        Address::Mailbox(Mailbox { dname: Some("Who?".into()),
                                   address: SMTPMailbox(DotString("one".into()).into(), "y.test".parse().unwrap())}),
    ]);
}
...
fn folded_qs() {
    ...
    assert_eq!(parsed.address, SMTPMailbox(DotString("mary".into()).into(), "x.test".parse().unwrap()));
}
