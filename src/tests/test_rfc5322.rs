...
#[test]
fn concat_atom() {
    assert_eq!(parse_single(from::<Intl>, b" atom                 <ignored@example>").dname, Some("atom".into()));
    assert_eq!(parse_single(from::<Intl>, b" atom  atom           <ignored@example>").dname, Some("atom atom".into()));
    assert_eq!(parse_single(from::<Intl>, b" atom  atom   atom    <ignored@example>").dname, Some("atom atom atom".into()));
}

#[test]
fn concat_qs() {
    let parsed = parse_single(from::<Intl>, b"\"no\"   \"space\"   space space \"two  space\" \"end space \" <ignored@example>\r\n");
...
fn simple_from() {
    ...
    assert_eq!(parsed.address, SMTPMailbox(DotAtom("jdoe".into()).into(), dp("machine.example")))
}
...
fn simple_sender() {
    ...
    if let Address::Mailbox(Mailbox{dname, address}) = parsed {
        ...
        assert_eq!(address, SMTPMailbox(DotAtom("mjones".into()).into(), dp("machine.example")))
    } else {
        unreachable!();
    }
}
...
#[test]
fn simple_reply_to() {
    let parsed = parse_single(reply_to::<Intl>, b"\"Mary Smith: Personal Account\" <smith@home.example>\r\n");
    assert_eq!(parsed.dname, Some("Mary Smith: Personal Account".into()));
    assert_eq!(parsed.address, SMTPMailbox(DotAtom("smith".into()).into(), dp("home.example")))
}
...
#[test]
fn multi_reply_to() {
    let (rem, parsed) = reply_to::<Intl>(b"Mary Smith <mary@x.test>, jdoe@example.org, Who? <one@y.test>\r\n").unwrap();
    assert_eq!(rem.len(), 0);
    assert_eq!(parsed, [
...
#[test]
fn folded_qs() {
    let parsed = parse_single(reply_to::<Intl>, b"\"Mary\r\n Smith\"\r\n <mary@\r\n x.test(comment)>\r\n");
    assert_eq!(parsed.dname, Some("Mary Smith".into()));
    assert_eq!(parsed.address, SMTPMailbox(DotAtom("mary".into()).into(), dp("x.test")));
}
